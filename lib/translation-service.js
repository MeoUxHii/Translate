async function translateText(text, targetLangOverride = null, isImageAnalysis = false) {
    try {
      ...
      if (service === "gemini") {
        ...
        if (targetLangOverride && typeof RETRANSLATE_PROMPTS !== 'undefined' && RETRANSLATE_PROMPTS[targetLangOverride]) {
             ...
             translationPrompt = isImageAnalysis
              ? buildImageAnalysisPrompt(text, targetLang, tone)
  ...
  async function handleImageTranslation(imageUrl, tabId) {
    try {
      ...
      const translationResult = await translateText(extractedText, null, true);
